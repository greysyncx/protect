#!/usr/bin/env bash
# GreySync Protect v1.6.4 (Clean Version)
...
php_bin(){ command -v php >/dev/null 2>&1 && echo php || echo ""; }
PHP="$(php_bin || true)"
php_check(){ [[ -n "$PHP" && -f "$1" ]] && ! "$PHP" -l "$1" >/dev/null 2>&1 && return 1 || return 0; }

backup_file(){ [[ -f "$1" ]] && mkdir -p "$BACKUP_DIR/$(dirname "${1#$ROOT/}")" && cp -af "$1" "$BACKUP_DIR/${1#$ROOT/}.bak"; }
restore_latest(){ [[ -L "$BACKUP_LATEST_LINK" ]] && dir="$(readlink -f "$BACKUP_LATEST_LINK")" || dir="$(ls -td $BACKUP_PARENT/greysync_* 2>/dev/null | head -n1 || true)"; [[ -d "$dir" ]] && find "$dir" -name "*.bak" | while read -r f; do cp -af "$f" "$ROOT/${f#$dir/%.bak}"; done; }
...
patch_file_manager(){
  ...
  insert_guard "$file" 'public\s+function\s+(index|download|contents|store|rename|delete)\s*\([^)]*\)\s*\{' "$guard" "GREYSYNC_PROTECT_FILE"
...
insert_guard_generic(){
  ...
  insert_guard "$file" 'public\s+function\s+(index|view|show|edit|update|create)\s*\([^)]*\)\s*\{' "$guard" "GREYSYNC_PROTECT_${tag}"
  log "$tag patched"
}
...
uninstall_all(){ rm -f "$STORAGE" "$IDPROTECT"; restore_latest; fix_laravel; log "Protect uninstalled"; }
