...
ensure_auth_use(){
  ...
  if [[ -n "$ln" ]]; then
    ...
    log "🔧 Added Illuminate\\Support\\Facades\\Auth to $f"
  fi
}
...
insert_guard_after_open_brace_multiline(){
  ...
  [[ -z "$line" ]] && { warn "⚠️ Function pattern not found in $f ($func_regex)"; return 1; }
  ...
  log "✅ Inserted guard in $f (line $insert_line)"
}
