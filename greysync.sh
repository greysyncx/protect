...
# Adds file-manager protection: only super-admin & server owner can access file contents.
# Usage:
...
YARN_BUILD=false   # ❗ Tidak perlu yarn build karena patch hanya di PHP
VERSION="1.6.3"
IN_INSTALL=false
EXIT_CODE=0
...
patch_file_manager(){
  ...
  [[ -f "$file" ]] || { log "Skip FileController"; return; }
  ...
  {print}
  ' "$file" > "$file.tmp" && mv "$file.tmp" "$file"
  php_check_file "$file" || { err "FileController syntax error"; return 1; }
