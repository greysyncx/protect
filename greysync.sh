...
if [ "$OPSI" = "1" ]; then
    ...
    in_func==1 && /^\s*{/ {
        ...
        print "            throw new DisplayException(\"Akses ditolak! Hanya Admin ID " admin_id " yang dapat menghapus user. ©GreySync v'"$VERSION"'\");";
        print "        }";
        in_func=0; next;
    }
    { print; }
    ...
    echo -e "${GREEN}✅ Protect ditambahkan ke UserController.${RESET}"
    ...
    in_func==1 && /^\s*{/ {
        ...
        print "            throw new DisplayException(\"Akses ditolak! Hanya Admin ID " admin_id " yang dapat menghapus server. ©GreySync v'"$VERSION"'\");";
        print "        }";
        in_func=0; next;
    }
    { print; }
    ...
    echo -e "${GREEN}✅ Protect ditambahkan ke ServerDeletionService.${RESET}"

    echo -e "${YELLOW}⚙️  Install Node.js 16 dan build panel...${RESET}"
...
# ================= OPSI 2 =======================
    ...
    echo -e "${YELLOW}♻ Memulihkan dari backup dan build panel...${RESET}"
...
# ============= OPSI 3 =====================
    ...
    echo -e "${YELLOW}🔗 Menjalankan Protect Admin eksternal (ireng.sh)...${RESET}"
    bash <(curl -s https://raw.githubusercontent.com/greysyncx/protect/main/greyz.sh)


# =========== OPSI 4 ==================
elif [ "$OPSI" = "4" ]; then
    echo -e "${CYAN}🗑 Menghapus patch Protect (uninstall) tanpa rebuild panel...${RESET}"

    if [ -f "$BACKUP_DIR/UserController.php.bak" ]; then
        cp "$BACKUP_DIR/UserController.php.bak" "$CONTROLLER_USER"
        echo -e "${GREEN}✔ UserController dipulihkan.${RESET}"
    else
        echo -e "${RED}⚠ Backup UserController tidak ditemukan.${RESET}"
    fi

    if [ -f "$BACKUP_DIR/ServerDeletionService.php.bak" ]; then
        cp "$BACKUP_DIR/ServerDeletionService.php.bak" "$SERVICE_SERVER"
        echo -e "${GREEN}✔ ServerDeletionService dipulihkan.${RESET}"
    else
        echo -e "${RED}⚠ Backup ServerDeletionService tidak ditemukan.${RESET}"
    fi

    echo -e "${GREEN}✅ Uninstall Protect selesai. Panel tidak di-rebuild.${RESET}"
